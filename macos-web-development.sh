...
if $HELP; then
    ...
    echo -e "${C_INFO}--no-apache        ${C_GOOD}Skip Apache${C_0}"
    echo -e "${C_INFO}--no-dnsmasq       ${C_GOOD}Skip Dnsmasq${C_0}"
    echo -e "${C_INFO}--no-dry-run       ${C_GOOD}Disable dry run and actually install stuff${C_0}"
    echo -e "${C_INFO}--no-mysql         ${C_GOOD}Skip MySQL${C_0}"
    echo -e "${C_INFO}--no-php           ${C_GOOD}Skip PHP${C_0}"
    ...
    echo -e "${C_INFO}--no-php-enable    ${C_GOOD}Will not enable the latest PHP version installed${C_0}"
    echo -e "${C_INFO}--no-xcode-select  ${C_GOOD}Skip xcode-select${C_0}"
    echo -e "${C_INFO}--uninstall        ${C_GOOD}Uninstall stuff, but leave some minor stuff${C_0}"
    echo ""
    exit
fi
