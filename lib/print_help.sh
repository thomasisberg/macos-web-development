...
print_help ()
{
    ...
    echo -e "${C_EM} • PHP versions 5.6 to 8.4 (optional)${C_0}"
    ...
    echo ""
    echo -e "${C_1}After installation you can run ${C_0}sphp 8.4${C_1} to switch PHP version (use desired version). ${C_0}sphp${C_1} only works with Apache from Homebrew.${C_0}"
    echo ""
    ...
    echo -e "${C_INFO}--no-php-8-2          ${C_EM}Skip PHP 8.2${C_0}"
    echo -e "${C_INFO}--no-php-8-3          ${C_EM}Skip PHP 8.3${C_0}"
    echo -e "${C_INFO}--no-php-8-4          ${C_EM}Skip PHP 8.4${C_0}"
    echo -e "${C_INFO}--no-php-8-5          ${C_EM}Skip PHP 8.5${C_0}"
    ...
    echo -e "${C_INFO}--only-php-8-2        ${C_EM}Only install PHP 8.2. Combine with --only-php if desired${C_0}"
    echo -e "${C_INFO}--only-php-8-3        ${C_EM}Only install PHP 8.3. Combine with --only-php if desired${C_0}"
    echo -e "${C_INFO}--only-php-8-4        ${C_EM}Only install PHP 8.4. Combine with --only-php if desired${C_0}"
    echo -e "${C_INFO}--only-php-8-5        ${C_EM}Only install PHP 8.5. Combine with --only-php if desired${C_0}"
    echo -e "${C_INFO}--only-xcode-select   ${C_EM}Only install xcode-select${C_0}"
    echo -e "${C_INFO}--p-common            ${C_EM}Sets options ${C_0}--only-php-8.4 --no-xcode-select"
    echo -e "${C_INFO}--p-minimal           ${C_EM}Sets options ${C_0}--no-mysql --only-php-8-4 --no-xcode-select"
